...
public class JiraProjectVersionEndpoint extends AbstractJiraEndpoint {
	...
	public Producer createProducer() throws Exception {
		final ProjectRestClient restClient = createClient().getProjectClient();
		final ProgressMonitor monitor = defaultProgressMonitor();
		final String projectKey = getProject();
		return new DefaultProducer(this) {
			public void process(Exchange exchange) throws Exception {
				exchange.getIn().setBody(
						restClient.getProject(projectKey, monitor).getVersions());
			}
		};
	}
